...
- (void)extractDMG
{
  @autoreleasepool
  {
    ...
    if([self isEncrypted:result] && [self.delegate respondsToSelector:@selector(unarchiver:requiresPasswordReturnedViaInvocation:)]) {
...
- (void)extractDMGWithPassword:(NSString *)password
{
  ...
  @autoreleasepool
  {
    ...
    if (taskResult != 0)
    {
      ...
      if (password != nil && [resultStr rangeOfString:@"Authentication error"].location != NSNotFound && [self.delegate respondsToSelector:@selector(unarchiver:requiresPasswordReturnedViaInvocation:)]) {
        [self performSelectorOnMainThread:@selector(requestPasswordFromDelegate) withObject:nil waitUntilDone:NO];
        goto finally;
      } else {
        NSLog( @"hdiutil failed with code: %lu data: <<%@>>", taskResult, resultStr );
        goto reportError;
      }
    }
...
- (void)requestPasswordFromDelegate
{
    ...
    [self.delegate unarchiver:self requiresPasswordReturnedViaInvocation:invocation];
}
